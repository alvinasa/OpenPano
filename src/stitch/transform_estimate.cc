...
TransformEstimation::TransformEstimation(const MatchData &m_match,
    ...
    : match(m_match), kp1(kp1), kp2(kp2), shape1(shape1), shape2(shape2),
    ...
    ransac_inlier_thres =
        (shape1.w + shape1.h) * 0.5 / 800 * RANSAC_INLIER_THRES;
}
...
Homography
TransformEstimation::calc_transform(const vector<int> &matches) const {
    vector<Vec2D> p1, p2;
    for (auto &i : matches) {
        p1.emplace_back(kp1[match.data[i].first]);
        p2.emplace_back(kp2[match.data[i].second]);
    }
    // return ((transform_type == Affine) ? getAffineTransform :
    // getPerspectiveTransform)(p1, p2);

    // normalize the coordinates before DLT, so that points are zero-centered
    // and have sqrt(2) mean distance to origin. suggested by MVG Sec 4.4
    auto normalize = [](vector<Vec2D> &pts) {
        ...
        for (const auto &p : pts)
            sqrsum += p.sqr() * sizeinv;
        double div_inv = sqrt(2.0 / sqrsum);
        for (auto &p : pts)
            p *= div_inv;
        return make_pair(mean, div_inv);
    };
    auto param1 = normalize(p1), param2 = normalize(p2);

    // homo from p2 to p1
    Matrix homo =
        ((transform_type == Affine) ? getAffineTransform
                                    : getPerspectiveTransform)(p1, p2);

    Homography t1{{param1.second, 0, -param1.second * param1.first.x, 0,
                   param1.second, -param1.second * param1.first.y, 0, 0, 1}};
    Homography t2{{param2.second, 0, -param2.second * param2.first.x, 0,
                   param2.second, -param2.second * param2.first.y, 0, 0, 1}};
...
vector<int> TransformEstimation::get_inliers(const Homography &trans) const {
    ...
    Matrix transformed =
        f2_homo_coor.prod(trans.transpose().to_matrix()); // nx3
    REP(i, n) {
...
bool TransformEstimation::fill_inliers_to_matchinfo(
    ...
    if (r1m < INLIER_IN_MATCH_RATIO || r1m > 1)
        return false;
    ...
    if (r2m < INLIER_IN_MATCH_RATIO || r2m > 1)
        return false;
    ...
    for (auto &idx : inliers)
        info->match.emplace_back(kp1[match.data[idx].first],
