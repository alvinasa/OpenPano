...
#pragma omp parallel for schedule(dynamic)
    REPD(i, mid - 1, 0) {
        ...
        MatchInfo info;
        bool succ =
            TransformEstimation(matches[i], keypoints[i + 1], keypoints[i],
                                imgs[i + 1].shape(), imgs[i].shape())
                .get_transform(&info);
        // Can match before, but not here. This would be a bug.
        if (!succ)
            error_exit(
                ssprintf("Failed to match between image %d and %d.", i, i + 1));
        // homo: operate on half-shifted coor
        bundle.component[i].homo = info.homo;
    }
    REPD(i, mid - 2, 0)
    bundle.component[i].homo =
        bundle.component[i + 1].homo * bundle.component[i].homo;
    bundle.calc_inverse_homo();
}

float CylinderStitcher::update_h_factor(float nowfactor, float &minslope,
                                        float &bestfactor,
...
#pragma omp parallel for schedule(dynamic)
    REPL(k, 1, len) {
        MatchInfo info;
        bool succ = TransformEstimation(matches[k - 1 + mid], nowkpts[k - 1],
                                        nowkpts[k], nowimgs[k - 1], nowimgs[k])
                        .get_transform(&info);
        if (!succ)
            failed = true;
    ...
    REPL(k, 1, len - 1)
    nowmat[k] =
        nowmat[k - 1] * nowmat[k]; // transform to nowimgs[0] == imgs[mid]
...
Mat32f CylinderStitcher::perspective_correction(const Mat32f &img) {
    ...
    vector<Vec2D> corners_std = {Vec2D(0, 0), Vec2D(0, h), Vec2D(w, 0),
                                 Vec2D(w, h)};
    ...
    blender.add_image(Coor(0, 0), Coor(w, h), tmp, [=](Coor c) -> Vec2D {
