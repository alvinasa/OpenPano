  ...
  public:
    ...
    struct ImageToAdd {
        ...
        Vec2D map_coor(int r, int c) const {
            ...
            if (ret.x < 0 || ret.x >= imgref.width() || ret.y < 0 ||
                ret.y >= imgref.height())
                ret = Vec2D::NaN();
            return ret;
        }
    ...
    // coor_func: the function maps from target coordinate to original image
    // coordinate.
    virtual void add_image(const Coor &upper_left, const Coor &bottom_right,
                           ImageRef &img,
