...
class MultiBandBlender : public BlenderBase {
    struct WeightedPixel {
        ...
        WeightedPixel operator*(float v) const {
            return WeightedPixel{w * v, c * v};
        }
        void operator+=(const WeightedPixel &p) {
            w += p.w;
            c += p.c;
        }
    };
    ...
    struct Mask2D {
        ...
        Mask2D(int hh, int ww)
            : w{(ww + 7) / 8 * 8}, // 8bit-aligned, so rows doesn't share bytes.
                                   // This allows concurrent access among rows.
              mask(hh * w, false) {}
  ...
  public:
    MultiBandBlender(int band_level) : band_level(band_level) {} // default: 5?

    void add_image(const Coor &upper_left, const Coor &bottom_right,
                   ImageRef &img, std::function<Vec2D(Coor)>) override;
