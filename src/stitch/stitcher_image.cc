...
void ConnectedImages::shift_all_homo() {
    ...
    Homography t2 =
        Homography::get_translation(component[mid].imgptr->width() * 0.5,
                                    component[mid].imgptr->height() * 0.5);
    REP(i, (int)component.size())
    if (i != mid) {
        Homography t1 =
            Homography::get_translation(component[i].imgptr->width() * 0.5,
...
void ConnectedImages::update_proj_range() {
    ...
    REP(i, CORNER_SAMPLE)
    REP(j, CORNER_SAMPLE) corner.emplace_back((double)i / CORNER_SAMPLE - 0.5,
                                              (double)j / CORNER_SAMPLE - 0.5);
    ...
    for (auto &m : component) {
        Vec2D now_min(numeric_limits<double>::max(),
...
Vec2D ConnectedImages::get_final_resolution() const {
    ...
    Vec2D resolution =
              id_img_range /
              Vec2D(refw, refh), // output-x-per-input-pixel, y-per-pixel
...
Mat32f ConnectedImages::blend() const {
    ...
    // dynamic_cast<LinearBlender*>(blender.get())->debug_run(size.x, size.y);
    // // for debug
    return blender->run();
}
