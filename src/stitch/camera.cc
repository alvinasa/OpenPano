...
double Camera::estimate_focal(const vector<vector<MatchInfo>> &matches) {
    int n = matches.size();
    vector<double> estimates;
    REP(i, n) REPL(j, i + 1, n) {
        auto &match = matches[i][j];
        if (match.confidence < EPS)
            continue;
        estimates.emplace_back(get_focal_from_matrix(match.homo));
    }
...
void Camera::rotation_to_angle(const Homography &r, double &rx, double &ry,
                               double &rz) {
...
void Camera::angle_to_rotation(double rx, double ry, double rz, Homography &r) {
    ...
    double u_outp[] = {rx * rx, rx * ry, rx * rz, rx * ry, ry * ry,
    ...
    double c = cos(theta), s = sin(theta), c1 = 1 - c;
    r.mult(c);
    REP(k, 9)
