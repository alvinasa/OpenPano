...
CameraEstimator::CameraEstimator(std::vector<std::vector<MatchInfo>> &matches,
                                 const std::vector<Shape2D> &image_shapes)
    : n(matches.size()), matches(matches), shapes(image_shapes),
...
vector<Camera> CameraEstimator::estimate() {
    ...
    traverse(
        ...
        [&](int now, int next) {
            ...
            // this is camera extrincis R, i.e. going from identity to this
            // image
            ...
            // may be deficiency in BA, or because of ignoring large error for
            // now
...
void CameraEstimator::traverse(function<void(int)> callback_init_node,
    ...
    if (cnt != n) {
        string unconnected;
        REP(i, n) if (not vst[i]) unconnected += to_string(i) + " ";
        error_exit(ssprintf(
