...
using namespace std;
namespace pano {
void MultiBandBlender::add_image(const Coor &upper_left,
                                 const Coor &bottom_right, ImageRef &img,
                                 std::function<Vec2D(Coor)> coor_func) {
    images_to_add.emplace_back(
        ImageToAdd{Range{upper_left, bottom_right}, img, coor_func});
...
void MultiBandBlender::create_first_level() {
    ...
    meta_images.reserve(
        nr_image); // we will need reference to this vector element
    ...
    REP(k, nr_image) {
        ...
        REP(i, range.height()) REP(j, range.width()) {
            ...
            if (c.get_min() < 0) { // Color::NO
                wimg.at(i, j).w = 0;
                wimg.at(i, j).c =
                    Color::BLACK; // -1 will mess up with gaussian blur
                mask.set(i, j);
            } else {
                wimg.at(i, j).c = c;
                ...
                wimg.at(i, j).w =
                    std::max(0.0, (0.5f - fabs(orig_coor.x)) *
                                      (0.5f - fabs(orig_coor.y))) +
                    EPS;
                // ext? eps?
            }
        }
...
#pragma omp parallel for schedule(dynamic)
        REP(i, target_size.y) REP(j, target_size.x) {
            ...
            REP(imgid, images.size()) {
                ...
                if (not img_cur.meta.range.contain(i, j))
                    continue;
                if (not img_cur.valid_on_target(j, i))
                    continue;
            ...
            if (not target_mask.get(i,
                                    j)) { // first time to visit *p. Note that
                                          // *p could be negative after visit.
                isum.write_to(p);
                target_mask.set(i, j);
            } else {
