...
class CylinderProject {
  public:
    ...
    CylinderProject(int m_r, const Vec &m_center, int m_size)
        : center(m_center), r(m_r), sizefactor(m_size) {}
  ...
  private:
    ...
    inline Vec2D proj(const Vec2D &p) const { return proj(Vec(p.x, p.y, 0)); }
...
class CylinderWarper {
  public:
    explicit CylinderWarper(real_t m_hfactor) : h_factor(m_hfactor) {}

    // warp image together with key points
    void warp(Mat32f &mat, std::vector<Vec2D> &kpts) const {
        mat = get_projector(mat.width(), mat.height()).project(mat, kpts);
    }
