...
// homo: from 2 to 1
std::vector<Vec2D> overlap_region(const Shape2D &shape1, const Shape2D &shape2,
...
class Homography {
  public:
    ...
    Homography(const double (&arr)[9]) { REP(i, 9) data[i] = arr[i]; }
    ...
    Homography transpose() const {
        return Homography{{data[0], data[3], data[6], data[1], data[4], data[7],
    ...
    inline Vec trans(const Vec2D &m) const { return trans(Vec(m.x, m.y, 1)); }

    inline Vec trans(double x, double y) const { return trans(Vec(x, y, 1)); }
    ...
    static bool health(const double *mat) {
        ...
        Vec x0{mat[2], mat[5], mat[8]},                            // trans(0,0)
            x1{mat[1] + mat[2], mat[4] + mat[5], mat[7] + mat[8]}; // trans(0,1)
        if (x1.y <= x0.y)
            return false;
        Vec x2{mat[0] + mat[1] + mat[2], mat[3] + mat[4] + mat[5],
    ...
    inline bool health() const { return Homography::health(data); }
