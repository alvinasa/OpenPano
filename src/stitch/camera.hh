...
class Camera {
  public:
    ...
    static double
    estimate_focal(const std::vector<std::vector<MatchInfo>> &matches);

    static void rotation_to_angle(const Homography &r, double &rx, double &ry,
                                  double &rz);

    static void angle_to_rotation(double rx, double ry, double rz,
                                  Homography &r);

    static void straighten(std::vector<Camera> &);

    friend std::ostream &operator<<(std::ostream &os, const Camera &c) {
        os << "focal=" << c.focal << ", ppx=" << c.ppx << ", ppy=" << c.ppy
           << ", R=" << c.R;
        double rx, ry, rz;
