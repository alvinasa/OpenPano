...
class StitcherBase {
  protected:
    // helper template to prevent generic constructor being used as copy
    // constructor
    template <typename A, typename B>
    using disable_if_same_or_derived = typename std::enable_if<!std::is_base_of<
        A, typename std::remove_reference<B>::type>::value>::type;
    ...
    std::vector<std::vector<Vec2D>>
        keypoints; // store coordinates in [-w/2,w/2]
  ...
  public:
    ...
    template <typename U,
              typename X = disable_if_same_or_derived<StitcherBase, U>>
    StitcherBase(U &&i) {
        /*
         *if (imgs.size() <= 1)
         *  error_exit(ssprintf("Cannot stitch with only %lu images.",
         *imgs.size()));
         */
        for (auto &n : i)
            imgs.emplace_back(n);
