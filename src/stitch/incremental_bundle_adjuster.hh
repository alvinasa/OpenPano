...
class IncrementalBundleAdjuster {
  public:
    ...
    IncrementalBundleAdjuster(std::vector<Camera> &cameras);

    IncrementalBundleAdjuster(const IncrementalBundleAdjuster &) = delete;
    IncrementalBundleAdjuster &
    operator=(const IncrementalBundleAdjuster &) = delete;
    ...
    ErrorStats get_error_stat() {
        ParamState state;
        for (auto &c : result_cameras)
            state.cameras.emplace_back(c);
        state.ensure_params();
  ...
  protected:
    ...
    struct MatchPair {
        ...
        MatchInfo &m;
        MatchPair(int i, int j, MatchInfo &m) : from(i), to(j), m(m) {}
    };
    ...
    Eigen::VectorXd get_param_update(const ParamState &state,
