...
static inline Vec2D homo2proj(const Vec &homo) {
    return Vec2D(atan2(homo.x, homo.z), homo.y / (hypot(homo.x, homo.z)));
}
...
// x <- x \pm pi
static inline Vec2D homo2proj(const Vec &homo) {
    return Vec2D(atan2(homo.x, homo.z), atan2(homo.y, hypot(homo.x, homo.z)));
}

// input & gradInput
// given h & dh/dx, return dp/dx = dp/dh * dh/dx
static inline Vec2D gradproj(const Vec &homo, const Vec &gradhomo) {
    double h_xz = homo.x * homo.x + homo.z * homo.z, h_xz_r = sqrt(h_xz),
           h_xyz_inv = 1.0 / (h_xz + homo.y * homo.y), h_xz_inv = 1.0 / h_xz;
    return Vec2D{gradhomo.x * homo.z * h_xz_inv -
                     gradhomo.z * homo.x * h_xz_inv,
