...
Homography Homography::inverse(bool *succ) const {
    ...
    Homography ret;
    auto res = to_eigenmap(ret), input = to_eigenmap(*this);
...
Homography Homography::operator*(const Homography &r) const {
    Homography ret;
    auto m1 = to_eigenmap(*this), m2 = to_eigenmap(r), res = to_eigenmap(ret);
    res = m1 * m2;
    return ret;
}
...
std::vector<Vec2D> overlap_region(const Shape2D &shape1, const Shape2D &shape2,
    ...
    // use sampled edge points, rather than 4 corner, to deal with distorted
    // homography for distorted homography, the range of projected z coordinate
    // contains 0 or equivalently, some point is projected to infinity
    ...
    REP(i, 4 * NR_POINT_ON_EDGE) {
        ...
        Vec2D pin1{transformed_pts.at(0, i) * denom,
                   transformed_pts.at(1, i) * denom};
