...
OrientationAssign::OrientationAssign(const DOGSpace &dog, const ScaleSpace &ss,
...
std::vector<float> OrientationAssign::calc_dir(const SSPoint &p) const {
    ...
    REP(i, ORI_HIST_BIN_NUM) {
        ...
        if (hist[i] > thres && hist[i] > max(prev, next)) {
            ...
            real_t newbin =
                (float)i - 0.5 + (hist[i] - prev) / (prev + next - 2 * hist[i]);
            // I saw this elsewhere... although by derivation the above one
            // should be correct
            // real_t newbin = (float)i - 0.5 + (next - prev) / (prev + next - 2
            // * hist[i]);
            if (newbin < 0)
