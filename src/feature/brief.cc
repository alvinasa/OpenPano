...
vector<Descriptor> BRIEF::get_descriptor() const {
    ...
    for (auto &p : points) {
        ...
        if (x >= half && x + half < img.width() && y >= half &&
            y + half < img.height()) {
...
Descriptor BRIEF::calc_descriptor(const SSPoint &p) const {
    ...
    REP(i, n) {
        int p1 = pattern.pattern[i].first, p2 = pattern.pattern[i].second;
        int y1 = y + p1 / pattern.s - half, x1 = x + p1 % pattern.s - half;
        int y2 = y + p2 / pattern.s - half, x2 = x + p2 % pattern.s - half;
