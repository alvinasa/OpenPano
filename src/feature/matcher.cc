...
#ifdef _MSC_VER
// necessary to define here since flann doesn't provide serialization for size_t
...
MatchData FeatureMatcher::match() const {
    static const float REJECT_RATIO_SQR =
...
#pragma omp parallel for schedule(dynamic)
    REP(k, l1) {
        ...
        float min = numeric_limits<float>::max(), next_min = min;
        ...
        // next_min remains a large-enough number here, don't need to
        // re-initialize
...
void PairWiseMatcher::build() {
    ...
    for (auto &feat : feats) {
        ...
        trees.emplace_back(
            points, flann::KDTreeIndexParams(FLANN_NR_KDTREE)); // TODO param
    }
#pragma omp parallel for schedule(dynamic)
...
MatchData PairWiseMatcher::match(int id1, int id2) const {
    ...
    trees[id2].knnSearch(query, indices, dists, 2,
                         flann::SearchParams(128)); // TODO param
    ...
    REP(i, source.size()) {
        ...
        trees[id1].knnSearch(query, indices_inv, dists_inv, 2,
