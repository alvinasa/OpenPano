...
void trilinear_interpolate(float xbin, float ybin, float hbin, float weight,
                           float hist[][DESC_HIST_BIN_NUM]) {
    // WARNING: x,y can be -1
    int ybinf = floor(ybin), xbinf = floor(xbin), hbinf = floor(hbin);
...
SIFT::SIFT(const ScaleSpace &ss, const vector<SSPoint> &keypoints)
...
Descriptor SIFT::calc_descriptor(const SSPoint &p) const {
    ...
    for (int xx = -radius; xx <= radius; xx++) {
        ...
        for (int yy = -radius; yy <= radius; yy++) {
            ...
            // -0.5 to make the center of bin 1st (x=1.5) falls fully into bin
            // 1st
            ...
            trilinear_interpolate(xbin, ybin, hist_bin, weight, hist);
        }
    }
