...
struct Descriptor {
    ...
    float euclidean_sqr(const Descriptor &r, float now_thres) const {
        return pano::euclidean_sqr(descriptor.data(), r.descriptor.data(),
                                   (int)descriptor.size(), now_thres);
    }

    int hamming(const Descriptor &r) const {
        return pano::hamming(descriptor.data(), r.descriptor.data(),
                             (int)descriptor.size());
    }
};
...
class FeatureDetector {
  public:
    ...
    virtual std::vector<Descriptor>
    do_detect_feature(const Mat32f &img) const = 0;
};
