...
ExtremaDetector::ExtremaDetector(const DOGSpace &dg) : dog(dg) {}
...
bool ExtremaDetector::calc_kp_offset(SSPoint *sp) const {
    ...
    for (; niter < CALC_OFFSET_DEPTH; ++niter) {
        if (!between(nowx, 1, w - 1) || !between(nowy, 1, h - 1) ||
            !between(nows, 1, nscale - 2))
            return false;

        auto iter_offset = calc_kp_offset_iter(now_pyramid, nowx, nowy, nows);
    ...
    sp->scale_id = nows;
    sp->scale_factor =
        GAUSS_SIGMA * pow(SCALE_FACTOR, ((double)nows + offset.z) / nscale);
    // accurate real-value coor
    sp->real_coor =
        Vec2D(((double)nowx + offset.x) / w, ((double)nowy + offset.y) / h);
    return true;
}

std::pair<Vec, Vec>
ExtremaDetector::calc_kp_offset_iter(const DOGSpace::DOG &now_pyramid, int x,
                                     int y, int s) const {
...
#define DS(x, y) now_scale.at(y, x)
    ...
    dxy = (DS(x + 1, y + 1) - DS(x + 1, y - 1) - DS(x - 1, y + 1) +
           DS(x - 1, y - 1)) /
          4;
    dys = (D(x, y + 1, s + 1) - D(x, y - 1, s + 1) - D(x, y + 1, s - 1) +
           D(x, y - 1, s - 1)) /
          4;
    dsx = (D(x + 1, y, s + 1) - D(x - 1, y, s + 1) - D(x + 1, y, s - 1) +
           D(x - 1, y, s - 1)) /
          4;
#undef D
#undef DS
...
bool ExtremaDetector::is_edge_response(Coor coor, const Mat32f &img) const {
    ...
    dxy = (img.at(y + 1, x + 1) + img.at(y - 1, x - 1) - img.at(y + 1, x - 1) -
           img.at(y - 1, x + 1)) /
          4;
    ...
    if (tr2 / det < sqr(EDGE_RATIO + 1) / EDGE_RATIO)
        return false;
    return true;
}

vector<Coor> ExtremaDetector::get_local_raw_extrema(int pyr_id,
                                                    int scale_id) const {
    vector<Coor> ret;
    ...
    REPL(i, 1, h - 1)
    REPL(j, 1, w - 1) if (is_extrema(i, j)) ret.emplace_back(j, i);
    return ret;
}
