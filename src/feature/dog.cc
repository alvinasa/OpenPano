...
GaussianPyramid::GaussianPyramid(const Mat32f &m, int num_scale)
    : nscale(num_scale), data(num_scale), mag(num_scale), ort(num_scale),
...
void GaussianPyramid::cal_mag_ort(int i) {
    ...
    ort[i] = Mat32f(h, w, 1);
    REP(y, h) {
        float *mag_row = mag[i].ptr(y), *ort_row = ort[i].ptr(y);
        const float *orig_row = orig.ptr(y), *orig_plus = orig.ptr(y + 1),
...
ScaleSpace::ScaleSpace(const Mat32f &mat, int num_octave, int num_scale)
    : noctave(num_octave), nscale(num_scale), origw(mat.width()),
      origh(mat.height()) {
    ...
    REP(i, noctave) {
        if (!i)
            ...
            int neww = ceil(origw * factor), newh = ceil(origh * factor);
...
Mat32f DOGSpace::diff(const Mat32f &img1, const Mat32f &img2) const {
    ...
    REP(i, h) {
        // speed up
        const float *p1 = img1.ptr(i), *p2 = img2.ptr(i);
        float *p = ret.ptr(i);
        REP(j, w)
        p[j] = fabs(p1[j] - p2[j]);
    }
...
DOGSpace::DOGSpace(ScaleSpace &ss)
    : noctave(ss.noctave), nscale(ss.nscale), origw(ss.origw), origh(ss.origh),
      dogs(noctave) {
#pragma omp parallel for schedule(dynamic)
    REP(i, noctave) {
