...
void test_extrema(const char *fname, int mode) {
    ...
    if (mode == 0) {
        ...
        for (auto &i : extrema) {
            Coor c{(int)(i.real_coor.x * mat.width()),
                   (int)(i.real_coor.y * mat.height())};
...
void test_orientation(const char *fname) {
    ...
    for (auto &i : oriented_keypoint)
        pld.arrow(
            Coor(i.real_coor.x * mat.width(), i.real_coor.y * mat.height()),
            i.dir, LABEL_LEN);
...
void test_match(const char *f1, const char *f2) {
    ...
    for (auto &x : ret.data) {
        pld.set_rand_color();
        Vec2D coor1 = feat1[x.first].coor, coor2 = feat2[x.second].coor;
        Coor icoor1 =
            Coor(coor1.x + pic1.width() / 2, coor1.y + pic1.height() / 2);
        Coor icoor2 = Coor(coor2.x + pic2.width() / 2 + pic1.width(),
                           coor2.y + pic2.height() / 2);
...
void test_inlier(const char *f1, const char *f2) {
    ...
    vector<Vec2D> kp1;
    for (auto &d : feat1)
        kp1.emplace_back(d.coor);
    vector<Vec2D> kp2;
    for (auto &d : feat2)
        kp2.emplace_back(d.coor);
    print_debug("Feature: %lu, %lu\n", feat1.size(), feat2.size());
    ...
    TransformEstimation est(ret, kp1, kp2, {pic1.width(), pic1.height()},
    ...
    for (auto &x : info.match) {
        pld.set_rand_color();
        Vec2D coor1 = x.first, coor2 = x.second;
        Coor icoor1 =
            Coor(coor1.x + pic1.width() / 2, coor1.y + pic1.height() / 2);
        Coor icoor2 =
            Coor(coor2.x + pic2.width() / 2, coor2.y + pic2.height() / 2);
        pld.circle(icoor1, LABEL_LEN);
    ...
    Shape2D shape2{pic2.width(), pic2.height()},
        shape1{pic1.width(), pic1.height()};
...
void planet(const char *fname) {
    ...
    REP(i, OUTSIZE) REP(j, OUTSIZE) {
        ...
        if (dist >= center || dist == 0)
            continue;
        dist = dist / center;
        // dist = sqr(dist);	// TODO you can change this to see different
        // effect
        dist = h - dist * h;
