...
Color interpolate(const Mat32f &mat, float r, float c) {
    ...
    if (*p < 0)
        return Color::NO; // return Color::NO if any one of the neighbor is
                          // Colo::NO
    ret += Color(p) * ((1 - r) * (1 - c));
    p = mat.ptr(fr + 1, fc);
    if (*p < 0)
...
Matrix getPerspectiveTransform(const std::vector<Vec2D> &p1,
                               const std::vector<Vec2D> &p2) {
    ...
    // TODO MVG suggests that this is more stable, but I found it a little bit
    // slower
    /*
     *MatrixXd m(n*2, 9);
     *REP(i, n) {
...
Matrix getAffineTransform(const std::vector<Vec2D> &p1,
                          const std::vector<Vec2D> &p2) {
    using namespace Eigen;
    int n = p1.size();
...
template <> void resize<float>(const Mat32f &src, Mat32f &dst) {
