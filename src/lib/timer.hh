...
class Timer {
  public:
    ...
    void restart() { m_start_time = std::chrono::high_resolution_clock::now(); }
  ...
  public:
    GuardedTimer(const std::string &msg, bool enabled = true)
        : GuardedTimer([msg](double duration) {
              std::cout << msg << ": " << std::to_string(duration * 1000.)
...
// record the total running time of a region across the lifecycle of the whole
// program call TotalTimer::print() before exiting main()
class TotalTimer {
  public:
    TotalTimer(const std::string &msg) : msg(msg) { timer.restart(); }
...
#define GUARDED_FUNC_TIMER GuardedTimer _long_long_name_guarded_timer(__func__)

#define TOTAL_FUNC_TIMER TotalTimer _long_long_name_total_timer(__func__)
