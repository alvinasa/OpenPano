...
#define PA(arr)                                                                \
    do {                                                                       \
        std::cout << #arr << ": ";                                             \
        std::copy(begin(arr), end(arr),                                        \
                  std::ostream_iterator<                                       \
                      std::remove_reference<decltype(arr)>::type::value_type>( \
                      std::cout, " "));                                        \
        std::cout << std::endl;                                                \
    } while (0)

void __m_assert_check__(bool val, const char *expr, const char *file,
...
void error_exit(const std::string &s) { error_exit(s.c_str()); }

// keep print_debug
#define print_debug(fmt, ...)                                                  \
    __print_debug__(__FILE__, __func__, __LINE__, fmt, ##__VA_ARGS__)

void __print_debug__(const char *file, const char *func, int line,
                     const char *fmt, ...)
