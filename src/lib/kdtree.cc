...
KDTree::Node *KDTree::build(vector<PointInBuild> &points, DimStats &stat,
                            int depth) {
    Split sp = stat.top();
    stat.pop();
    ...
    // split_axis, sp.min + sp.range * 0.5, points.size());
    Node *ret = new Node();
...
KDTree::NNResult KDTree::nn_in_node(const Point &p, Node *n,
                                    float thres) const {
    if (n->leaf()) {
        float min_d = thres;
        int min_i = -1;
        for (auto &lp : n->pts) {
            float v = euclidean_sqr(lp.second->data(), p.data(), D, min_d);
    ...
    float split = n->split, dist = p[n->axis] - split;
    int left = dist <= 0;
...
KDTree::TwoNNResult KDTree::two_nn_in_node(const Point &p, Node *n,
                                           float thres) const {
    if (n->leaf()) {
        float min_d = thres, min_d2 = thres;
        int min_i = -1;
        for (auto &lp : n->pts) { // leaf point
            float v = euclidean_sqr(lp.second->data(), p.data(), D, min_d2);
            if (v < min_d)
