...
#define REP(x, y)                                                              \
    for (typename std::remove_cv<                                              \
             typename std::remove_reference<decltype(y)>::type>::type x = 0;   \
         x < (y); x++)
#define REPL(x, y, z)                                                          \
    for (typename std::remove_cv<                                              \
             typename std::remove_reference<decltype(y)>::type>::type x = y;   \
         x < (z); x++)
#define REPD(x, y, z)                                                          \
    for (typename std::remove_cv<                                              \
             typename std::remove_reference<decltype(y)>::type>::type x = y;   \
...
__attribute__((format(printf, 1, 2))) std::string ssprintf(const char *fmt,
                                                           ...);

template <typename T> inline bool update_min(T &dest, const T &val) {
    if (val < dest) {
        dest = val;
        return true;
    }
...
template <typename T> inline bool update_max(T &dest, const T &val) {
    if (dest < val) {
        dest = val;
        return true;
    }
...
template <typename T> inline void free_2d(T **ptr, int w) {
    if (ptr != nullptr)
        for (int i = 0; i < w; i++)
            delete[] ptr[i];
    delete[] ptr;
